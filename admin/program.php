...
<?php
      if(isset($_GET['confirm'])) {
        ...
        echo info("Are you sure you want to delete program, <b>$row[name] ($row[id])</b>?");
  ...
  if(isset($_POST['submit'])){
    ...
    if($hname==""){
     ...
     $insert = $db->query("insert into program(name)values('$hname')");
   ...
   <?php
        if($query->num_rows>0) {
          ...
          while($row = $query->fetch_assoc()) {
            echo '<tr><td>'.$i.'</td><td>'.$row['name'].'</td> <td><a href="program.php?action=delete&id='.$row['id'].'">Delete</a></td></tr>';
            $i++;
          }
        } else {
